...
export const projects = [
  ...
  },
  {
    ...
    result: 'Reproduces handwriting from a camera image',
    body:
      'A 2-DOF arm with pen actuation. Forward and inverse kinematics plan the trajectory; OpenCV and Tesseract turn a camera frame into text, and the text becomes stroke-based motion paths the servos execute. Firmware in C/C++ handles PWM servo synchronization, and a TinyML model on the ESP32 does lightweight character detection on-device.',
    stack: ['C++', 'Python', 'ESP32', 'OpenCV', 'Tesseract', 'TinyML'],
    image: null,
    href: null,
  },
  {
    ...
    date: 'Aug 2025',
    domains: ['ml'],
    title: 'Lightweight ML Graph Optimizer',
    result: 'Measurable speedup over naive execution',
    body:
      'A toy compiler that parses simple compute graphs — MatMul into ReLU and similar — into a DAG, then applies constant folding and operator fusion. It emits C and NumPy kernels from the optimized graph, and I benchmarked those against naive execution to show where the fusion actually pays.',
    stack: ['C++', 'Python', 'DAG', 'Operator fusion'],
    image: null,
    href: null,
  },
  {
    ...
    result: '92% validation accuracy across three corn leaf diseases',
    body:
      'A CNN trained on an augmented dataset to classify three corn leaf diseases, then optimized for TinyML deployment so the diagnosis runs offline on an edge device. The constraint that mattered was the target, not the accuracy — a model that needs a server is no use in a field.',
